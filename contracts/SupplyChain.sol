...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
   ...
   */
  modifier forSale(uint _sku) { require(items[_sku].state == State.ForSale && items[_sku].price != 0, "item is not for sale"); _; }
  modifier sold(uint _sku) { require(items[_sku].state == State.Sold, "item has not been sold"); _; }
  modifier shipped(uint _sku) { require(items[_sku].state == State.Shipped, "item has not been shipped"); _; }
  modifier received(uint _sku) { require(items[_sku].state == State.Received, "item has not been received"); _; }
